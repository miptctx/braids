...
#perms = Permutations([x,x+1,x+2,x+3,x+4])
...
for z_1, z_2, z_3, z_4, z_5 in perms:
  ...
  t_3, m_3 = braiding_ext(t_0, (rotate_tuple(z_1, z_2, z_4, z_5),{z_1,z_4}), F=F)
  t_4, m_4 = braiding_ext(t_3, (rotate_tuple(z_2, z_3, z_4, z_5),{z_2,z_4}), F=F)
  t_5, m_5 = braiding_ext(t_4, (rotate_tuple(z_1, z_2, z_3, z_5),{z_2,z_5}), F=F)
  ...
  # print(latex((m_l[1][1].simplify_full() - m_r[1][1].simplify_full()).simplify_full()))
  # print(latex ((m_l[1][1] / m_r[1][1]).simplify_full() ))
  #print((m_l[1][1].simplify_full() / m_r[1][1].simplify_full()).simplify_full())

  lhs = m_r[0][0]._maxima_()#.fullratsimp()
  rhs = m_l[0][0]._maxima_()#.fullratsimp()

  print((lhs/rhs).ratsimp().sage())

  #result = maxima(f"ratsimp( {lhs.name()} / {rhs.name()} );").eval()
  #print(result.sage())
  ...
  print(m_l[0][0].simplify_full() == m_r[0][0].simplify_full())
  print("")
  print(m_l[0][0].simplify_full())
  print("")
  print(m_r[0][0].simplify_full())
  quit()

  print(m_l[0][0].simplify_full() == m_r[0][0].simplify_full())
  print(
    m_l[0][0].simplify_full().subs({a_1: 1, a_2: 2, a_3: 3, a_4: 4, a_5: 5}) == \
    m_r[0][0].simplify_full().subs({a_1: 1, a_2: 2, a_3: 3, a_4: 4, a_5: 5}))

  maxima(f"assume({z_1} > 0);")
  maxima(f"assume({z_1} < {z_2});")
  maxima(f"assume({z_2} < {z_3});")
  maxima(f"assume({z_3} < {z_4});")
  maxima(f"assume({z_4} < {z_5});")

  lhs = m_l[0][0].simplify_full()._maxima_().fullratsimp()
  rhs = m_r[0][0].simplify_full()._maxima_().fullratsimp()
  result = maxima(f"is(equal({lhs.name()}, {rhs.name()}));")
